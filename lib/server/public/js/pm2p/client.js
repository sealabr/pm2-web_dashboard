  ...
  return {
    ...
    enrichStats: function(systemStats){
      ...
      for (p in updatedStats.processes) {
        ...
        process.is_yowsup = process.name.toUpperCase().indexOf('YOWSUP') !== -1 ? 'show' : 'hide';
        process.is_messenger = process.name.toUpperCase().indexOf('MESSENGER') !== -1 ? 'show' : 'hide';

        process.host = systemStats.id;

        process.pm_id = process.pm2_env.pm_id;

        process.pm_out_log_path = process.pm2_env.pm_out_log_path;
        process.pm_err_log_path = process.pm2_env.pm_err_log_path;

        // PM2 Ecosystem Variables
        process.service_type =  process.pm2_env.service_type;
        process.service_id =  process.pm2_env.service_id;
        process.service_client =  process.pm2_env.service_client;
        process.service_port =  process.pm2_env.service_port;
        process.service_version = process.pm2_env.service_version;
